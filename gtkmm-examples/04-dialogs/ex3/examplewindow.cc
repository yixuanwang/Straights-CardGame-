...
ExampleWindow::ExampleWindow() : m_VBox{Gtk::ORIENTATION_VERTICAL, 5}, m_Button_Dialog{"Choose Color"} {
  ...
  m_ColorButton.signal_color_set().connect(sigc::mem_fun(*this, &ExampleWindow::on_color_button_color_set) );

  m_VBox.pack_start(m_Button_Dialog, Gtk::PACK_SHRINK);
  m_Button_Dialog.signal_clicked().connect(sigc::mem_fun(*this, &ExampleWindow::on_button_dialog_clicked) );
  ...
  m_DrawingArea.signal_draw().connect(sigc::mem_fun(*this, &ExampleWindow::on_drawing_area_draw));
