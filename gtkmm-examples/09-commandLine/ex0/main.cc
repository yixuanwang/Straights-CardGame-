// Had to be tweaked slightly to work with glibmm2.4 as installed in student environment.
    ...
    m_arg_foo{0}, m_arg_boolean{false}
{
    ...
    // We can choose a default to be used if the user doesn't specify this option.
    ...
    entry6.set_flags(Glib::OptionEntry::Flags::FLAG_OPTIONAL_ARG);
    ...
    entry7.set_flags(Glib::OptionEntry::Flags::FLAG_OPTIONAL_ARG);
...
bool ExampleOptionGroup::on_pre_parse(Glib::OptionContext& /* context */, Glib::OptionGroup & group ) {
    ...
    cout << "on_pre_parse called" << endl;
    return true;
} // ExampleOptionGroup::on_pre_parse
...
bool ExampleOptionGroup::on_post_parse( Glib::OptionContext& /* context */, Glib::OptionGroup & group ) {
    ...
    // You do not need to override this method. This is just here to show you how,
    // in case you want to do any extra processing.
    cout << "on_post_parse called" << endl;
    return true;
} // ExampleOptionGroup::on_post_parse

void ExampleOptionGroup::on_error( Glib::OptionContext& /* context */, Glib::OptionGroup & group ) {
    cout << "on_error called" << endl;
} // ExampleOptionGroup::on_error

bool ExampleOptionGroup::on_option_arg_string( const Glib::ustring& option_name, const Glib::ustring& value,
					       bool has_value)
{
...
bool ExampleOptionGroup::on_option_arg_filename( const Glib::ustring& option_name, const std::string& value,
						 bool has_value )
{
...
int main( int argc, char** argv ) {
    ...
    cout << "parsed values: " << endl
	      << "  foo = " << group.m_arg_foo << endl
	      << "  filename = " << group.m_arg_filename << endl
	      << "  activate_something = " << (group.m_arg_boolean ? "enabled" : "disabled")
	      << endl
	      << "  goo = " << group.m_arg_goo << endl
	      << "  x-string = " << group.m_arg_x_string << endl
	      << "  x-filename = " << group.m_arg_x_filename << endl;

    // This one shows the results of multiple instance of the same option, such as --list=1 --list=a
    // --list=b
    cout << "  list = ";
    for ( const auto & i : group.m_arg_list ) cout << i << ", ";
    cout << endl;

    // This one shows the remaining arguments on the command line, which had no name= form:
    cout << "  remaining = ";
    for ( const auto & i : group.m_remaining_list ) cout << i << ", ";
    cout << endl;
