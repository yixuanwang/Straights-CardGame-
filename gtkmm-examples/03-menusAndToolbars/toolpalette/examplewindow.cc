...
using type_stringvec = std::vector<Glib::ustring> ;
...
void ExampleWindow::load_icon_items() {
  ...
  for (auto iter : contexts) {
    ...
    Gtk::ToolItemGroup* group = Gtk::manage(new Gtk::ToolItemGroup(context_name));
    ...
    guint icons_count = 0;
    for (auto iconiter : icon_names_for_context) {
...
} // ExampleWindow::load_icon_items


void ExampleWindow::load_toggle_items() {
  auto group = Gtk::manage(new Gtk::ToolItemGroup("Radio Item"));
  m_ToolPalette.add(*group);
...
} // ExampleWindow::load_toggle_items
...
void ExampleWindow::load_special_items() {
  ...
  group->insert(*item);
  gtk_container_child_set (GTK_CONTAINER (group->gobj()), GTK_WIDGET (item->gobj()),
                           "homogeneous", FALSE, "expand", TRUE, nullptr);

  item = create_entry_item ("homogeneous=FALSE, expand=TRUE, fill=FALSE");
  group->insert(*item);
  gtk_container_child_set (GTK_CONTAINER (group->gobj()), GTK_WIDGET (item->gobj()),
                           "homogeneous", FALSE, "expand", TRUE, "fill", FALSE, nullptr);

  item = create_entry_item ("homogeneous=FALSE, expand=TRUE, new-row=TRUE");
  group->insert(*item);
  gtk_container_child_set (GTK_CONTAINER (group->gobj()), GTK_WIDGET (item->gobj()),
                           "homogeneous", FALSE, "expand", TRUE, "new-row", TRUE, nullptr);
...
ExampleWindow::ExampleWindow() : m_VBox{Gtk::ORIENTATION_VERTICAL, 6}, m_HBox{Gtk::ORIENTATION_HORIZONTAL, 6} {
